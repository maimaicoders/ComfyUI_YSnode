...
model = YOLO(task='detect', model='/root/autodl-tmp/ComfyUI/custom_nodes/comfyui-reactor-node/yolo_face_predict.pt')
...
class Mynode_2:
    ...
    def method(self, input_image, source_image):
        ...
        if judge_face == 0: # 等于0就是没检测出脸
            # PIL转Pytorch张量
            # torch_img = pil_to_tensor_grayscale(input_image_pil)
            return (input_image,)
